...
## New local harness: compare against amitools' own test/bin corpus, found 3 real bugs (#45-#47) — 2026-09-11

`tools/compare_amitools_suite.py`, a sibling to `compare_vamos.py`
(volamos's own ~6 hand-written fixtures) and `compare_three_way.py`
(the AmiBake/Kickstart three-way harness): runs amitools' *own* test
corpus at `~/src/amitools/test` -- ~40 small real C programs, each
pre-built with up to four real Amiga compilers, plus a pytest suite
pinning down expected output and checked-in ground-truth `.txt` files
for several -- against both volamos and `vamos`, local-only and never
bundled (amitools is GPLv2; volamos is MIT/Apache-2.0), same posture as
`compare_three_way.py`'s external AmiBake/Kickstart assets.

Each corpus entry supplies the *same* explicit `-V`/`-a` flags to both
engines rather than relying on either's own "zero-config" defaults,
which no longer agree by design since issue #43 (vamos's own test
suite assigns `sys:` to its `ram:` auto-volume with a `c`/`s`/`libs`/
`devs`/`t` skeleton; volamos's own defaults use a persistent `SYS:`
with `envarc`/`env` added and `t` under `RAM:` instead).

Initial 5-entry corpus (`dos_match`, `dos_seek`, `util_date`, `vprintf`,
`exec_rawdofmt` -- chosen for needing no companion-binary build or
extra toolchain) immediately found:

- **Issue #45** (real bug): `RawDoFmt`'s `%b` (BSTR) format reads the
  raw `u32` off the data stream and uses it directly as a byte address,
  never applying the BPTR->address shift (`addr_from_bptr`) every other
  BPTR consumer in the codebase already uses -- `execfmt.rs`.
- **Issue #46** (real bug): `dosanchor.rs`'s `strip_device_prefix`
  (added for issue #14, meant to deduplicate a doubled device prefix)
  actually strips the device prefix *entirely*, contradicting its own
  doc comment's worked example ("real hardware's `SYS:S/Shell-Startup`")
  and `MatchFirst`'s own NDK autodoc ("FULL PATH NAME" when
  `ap_Strlen != 0`) -- `ap_Buf` ends up bare-name-only, or completely
  empty for a volume-root entry.
- **Issue #47** (real bug): `Seek()` never rejects a target position
  beyond end-of-file, contradicting the NDK's own explicit "You cannot
  Seek() beyond the end of a file" -- `dosfile.rs` delegates straight
  to the host `seek()`, which (unlike real AmigaOS) allows it.
- **Issue #48** (needs real-hardware verification): `%x`/`%lx` hex
  digit case differs (volamos lowercase, vamos uppercase) -- no NDK
  autodoc pins this down either way.
- **Issue #49** (vamos's own divergence, not a volamos bug):
  `CheckDate()`'s `wday` field -- `utility.library/CheckDate`'s own NDK
  autodoc documents "the wday field ... is not checked" as a real
  historical bug; volamos matches that, vamos validates it anyway.

Also surfaced, while root-causing #48/#49: amitools' own
`test/data/*.txt` "ground truth" files are themselves generated by
capturing a `vamos` run (`test/helper/runner.py`'s `--gen-data` mode),
not independently verified against real hardware -- so for a genuinely
uncertain question, agreeing with the `.txt` file is really just
"agrees with vamos," one data point, not two. Documented directly in
the harness's own module doc as a standing caveat.

Deferred, per the plan this was scoped from: entries needing a
companion-binary build (`dos_loadseg`, `dos_runcmd`, `proc_args` with
real args) or another toolchain, the `sc`/`vc`/`agcc` flavor sweep, and
...
## Issue #55 fixed: dos.library/FindArg implemented — 2026-09-12

`FindArg(template, keyword)` was listed in the LVO table but had no registered handler at all -- an unhandled-library-call trap. Implemented on top of `dosargs.rs`'s existing `ReadArgs` template parser (`parse_template`/`matches_keyword`), which already recognized everything `FindArg` needs: a `NAME=ABBREV` template slot's two names, matched case-insensitively. New pure `find_arg(template, keyword) -> i32` function (mirroring the existing `read_args`/`free_args` split from their own `*_handler` wrappers) returns the zero-based slot index or `-1`; a malformed template has no separate error-reporting channel per `FindArg`'s own NDK autodoc (only an index or `-1` is documented), so it's treated the same as "not found" rather than propagating `ERROR_BAD_TEMPLATE`.

Confirmed directly against the real `dos_findarg_gcc` binary: all four cases (`a=b/k` matched by either name, an unmatched keyword, and a `/M` slot's index) now match amitools' own expected output exactly. Five new unit tests. `dos_findarg` is now a full three-way `PASS` in the harness -- the one remaining gap it exists to catch is closed.

## Issue #46 investigated, not fixed: conflicting real-hardware evidence, needs a genuine FFS-backed re-test

Attempted this issue's proposed fix (stop `strip_device_prefix`-ing `ap_Buf`, matching `vamos`'s and the NDK autodoc's "FULL PATH NAME" wording) and verified it against real Kickstart 3.1 hardware (Copperline, A600, 40.72 ROM) before committing to it, since it would directly revert issue #14's own real-hardware-confirmed fix. The two came back genuinely conflicting:

- Running amitools' own `dos_match_gcc` under Copperline's `--run` convenience boot (auto-mounts the executable's directory as `RunProg:`) showed `ap_Buf` clearly retaining the full device-qualified path (`c RunProg:c 64 65`, etc.) -- matching this issue's diagnosis and `vamos`'s output.
- Re-running #14's own exact repro (`Delete SYS:S/Shell-Startup`, real `wb314` `C:Delete` binary) under the same Copperline setup (`RunProg:victim.txt`) showed a **single**, non-doubled `Deleted` line -- even though `ap_Buf` for that match would, per the above, already carry the full prefix.
- Applying the proposed fix and re-running volamos's *own* `Delete SYS:S/Shell-Startup` against the exact scratch setup that originally confirmed #14 immediately **reintroduced #14's exact doubled-prefix bug** (`SYS:SYS:S/Shell-Startup  Deleted`) -- a direct, reproducible, in-repo regression, not a matter of interpretation.

Since Copperline's `--run`-provided `RunProg:`/`RunBoot:` volumes are documented as "virtual filesystem volumes using the host filesystem interface" -- plausibly Copperline's own simplified host-directory bridge, not necessarily identical to genuine AmigaDOS/FastFileSystem packet-level behavior -- the direct in-repo regression evidence was trusted over the `--run`-based hardware test, and the change was reverted rather than shipped on inconclusive grounds. #14's original verification used a real `[[filesys]]`-mounted disk volume through a full AmiBake OS 3.1.4 corpus (genuine FFS); resolving #46 properly needs `dos_match_gcc` re-tested the same way, not through `--run`'s convenience mount. Documented on the issue; left open, not closed.

## Issue #46 resolved: ap_Buf tracks the caller's own pattern qualification, not internal VFS-canonical form — 2026-09-12

Followed up on the previous entry's inconclusive investigation, using the technique the user pointed at directly: Copperline's `[ide]`/`[scsi]`/`[lide]` directory-mount (`master = { path = "...", filesystem = "ffs" }`), which synthesizes a *genuine* in-memory FFS/OFS volume with a real Rigid Disk Block at boot -- discarded on exit -- rather than `--run`'s convenience `RunProg:`/`RunBoot:` bridge or `[[filesys]]`'s own `HOSTFS<n>:` handler, neither of which necessarily exercises real `dos.library`/filesystem packet-level behavior.

Three genuine-FFS-backed real-hardware tests (Kickstart 40.72, A600) settled it definitively:

1. `dos_match_gcc sys:` (device-qualified pattern): `ap_Buf` entries are device-qualified (`sys:c`, `sys:devs`, ...) -- confirms #46's original claim, this time via a trustworthy method.
2. `Delete SYS:S/Shell-Startup` (device-qualified pattern): prints a single, non-doubled `SYS:S/Shell-Startup  Deleted` -- confirms #14's original repro still holds even with a fully-qualified `ap_Buf`.
3. `dos_match_gcc Shell-Startup` after `cd S` (**no** device or path prefix in the pattern at all): `ap_Buf` comes back completely unqualified, identical to `fib_FileName` (`Shell-Startup Shell-Startup ...`) -- even though the scan's own internal resolution is fully qualified (`SYS:S/`) to actually find the file.

Test 3 is the key that reconciles 1 and 2: **`ap_Buf`'s qualification level tracks whatever the caller's own pattern text specified, not this module's internal `VFS`-canonical form.** Real `Delete`'s own multi-step algorithm apparently issues a *second*, purely relative `MatchFirst` (after its own internal `CurrentDir()`) to build its display string -- on real hardware that second call's `ap_Buf` comes back unqualified (matching its own relative pattern), so concatenating it with a separately-scanned device prefix never doubles. volamos's previous implementation always fully-qualified `ap_Buf` regardless of the query's own qualification (`dir_part` for a bare pattern was always built from `Vfs::cwd()`), so its own equivalent internal second `MatchFirst` call was *also* over-qualified, and doubled when `strip_device_prefix` was removed -- explaining the earlier regression precisely.

Fixed properly this time: `ScanLevel` gained a `display_prefix` field, threaded independently of the existing (still fully-`VFS`-qualified, used for real host resolution) `dir_amiga_path` through every `APF_DODIR` descent level. `match_first` computes it alongside `dir_part`/`name_part`: the caller's own literal prefix text when the pattern had one, empty string when it didn't (instead of always falling back to `cwd()`). The non-wildcard branch's `ap_Buf` is now simply the caller's own literal pattern text verbatim (`pat_str`), not `entry.amiga_path`. `write_match_result` no longer strips anything -- `strip_device_prefix` is gone entirely, replaced by `display_path` already being correct at every call site.

Verified: `Delete SYS:S/Shell-Startup` still single-prefixed after this fix (the actual regression test that mattered), `dos_match`'s `ap_Buf` column now matches `vamos`'s exactly on every row. Two new unit tests (`ap_buf_has_no_device_qualifier_for_a_bare_cwd_relative_pattern`, `ap_buf_for_a_wildcard_scan_keeps_the_callers_own_device_prefix`) plus a rewritten one. `dos_match`'s only remaining mismatch is narrower and separately filed (#58): the volume-root rows' `fib_FileName` (confirmed via the same real-hardware tests to want blank, not `"SYS"`/`"sys:"`) and an `APF_DirChanged` flag question `own_display_name` -- a function shared with plain `Lock()`/`Examine()`, not just `AnchorPath` -- would need its own separate real-hardware check before changing. Full suite: 720 passing, clippy/fmt clean.

## Issue #58 (finding 1 of 2) fixed: AnchorPath's own fib_FileName is blank for a bare volume root — 2026-09-12

Verified via a dedicated real-hardware check (Copperline, genuine in-memory FFS via `[ide]`): a plain `Lock("SYS:")`/`Examine()` on a bare volume root reports `fib_FileName` as `"SYS"` (the volume name) -- confirming `doslock.rs`'s shared `own_display_name` is *already correct* for that call site and must not be touched. But `MatchFirst("sys:", ...)`'s own two volume-root reports both have a **blank** `fib_FileName` on the same real hardware -- a distinct convention specific to `AnchorPath`'s own reporting (matching the NDK autodoc's note that "patterns with trailing slashes may cause MatchFirst()/MatchNext() to return ... a filename of the empty string").

Fixed by adding a new `dosanchor.rs`-local `anchor_display_name` (same last-path-component logic as `own_display_name`, but returning `""` instead of falling back to the volume name when there's no non-empty component), used at `dosanchor.rs`'s own two `own_display_name` call sites instead of the shared `doslock.rs` function, which is now used only by its own `Examine()` path. New test: `fib_filename_is_blank_for_a_bare_volume_root_match`.

While re-verifying "finding 2" (the `APF_DirChanged` flag question) against this same real-hardware run, found it's not quite what the original filing assumed: the `"c"` entry's own `ap_Flags` (`65` = `APF_DIR_CHANGED | APF_DOWILD`) **already matches real hardware exactly** -- `vamos`'s `1` (no `DIR_CHANGED` bit) is the one that diverges from real hardware there, not volamos. The genuinely open question is narrower and structural: real hardware's volume-root scan reported the root entry **twice, both at the very end** (after all five children, back-to-back, flags `73` then `9`), not framing the children with one leading + one trailing report the way both `vamos` and volamos's current `AnchorMatchState`/`ScanLevel` state machine assume. That's a deeper change to the scan's own event sequence, not a flag-value tweak -- left open on #58 with this refined understanding rather than attempted this session. Full suite: 721 passing, clippy/fmt clean.

## mathieeesingbas.library/mathieeesingtrans.library implemented — 2026-09-12

Simon: "it sounds like it would be worth adding support for mathieeesingbas.library and mathieeesingtrans.library" -- these two were previously only fake-succeeding `OpenLibrary` stand-ins (`STANDARD_WORKBENCH_LIBRARIES`, no real LVOs), matching the amitools harness's own "real gap, not a quick add" deferral note for `math_single`/`math_single_trans`.

**LVO tables**: extracted from AROS's own `workbench/libs/{mathieeesingbas,mathieeesingtrans}/*.conf` via `tools/gen_lvos.py`, same commit (`c207aae1d67ac419553530b80eb62dcab2f923ee`) already used for the double-precision tables, `--start-bias 24`. Every offset is a mechanical `IEEEDP*` -> `IEEESP*` rename at the *same* LVO number (`IEEESPFix`/`IEEEDPFix` both `-30`, and so on down both lists) -- independently cross-checked against <https://anadoxin.org/blog/amigaos-stdlib-vector-tables.html/>, which lists the same values.

**`crates/volamos-core/src/mathlibs.rs`**: `ieeesp_*` handlers operate on plain `f32` (`from_bits`/`to_bits`, one register per value) mirroring `ieeedp_*`'s `f64`/register-pair shape function-for-function -- `Fix`/`Flt`/`Cmp`/`Tst`/`Abs`/`Neg`/`Add`/`Sub`/`Mul`/`Div`/`Floor`/`Ceil` (mathieeesingbas) and `Atan`/`Sin`/`Cos`/`Tan`/`Sincos`/`Sinh`/`Cosh`/`Tanh`/`Exp`/`Log`/`Pow`/`Sqrt`/`Tieee`/`Fieee`/`Asin`/`Acos`/`Log10` (mathieeesingtrans). `IEEESPCeil`/`ieeesp_unary`'s NaN canonicalization carry over the double-precision real-hardware findings (issues #51/#52) *by analogy*, not independently re-verified for singles. `IEEESPTieee`/`IEEESPFieee` are real documented no-ops (their own NDK autodoc: "included for completeness... just return the input parameter", since this library's native format already is IEEE single) -- `mathieeedoubtrans.library`'s equivalents do a real double<->single conversion, so this isn't just a copy-paste of that pair.

**Real bug found via `amitools`' own `math_single_trans` ground truth** (not from a saved/assumed value): `IEEESPPow`'s AROS `.conf` register order (`D1`, `D0` for the C prototype's `x`, `y`) is *not* the naive `D0`-then-`D1` every other single-precision two-argument call in these libraries uses -- already documented in the code before running the real binary. But the FIRST implementation attempt additionally got the base/exponent relationship backwards: `IEEESPPow(3.0, 4.0)` must give `64.0` (`4**3`), not `81.0` (`3**4`) -- confirmed directly against `test/data/math_single_trans.txt`. Tracing this down also turned up that `mathieeedoubtrans.library`'s existing, already-shipped `IEEEDPPow` has the *same* base/exponent relationship (`D0`/`D1` register pair is the real base, `D2`/`D3` the real exponent) -- it was already correct (matches `math_double_trans.txt`'s own `pow5: 3**4 -> 64` line, previously unnoticed since nothing had cross-checked the *labels*, only the pass/fail), just mislabeled in its own doc comment as `x.powf(y)` in the naive sense. Both doc comments corrected; `sp_pow_handler`'s FFP `SPPow` (mathtrans.library) has the same base/exponent shape too -- three-for-three across the FFP/single/double math libraries now.

**Address space**: `MATHIEEESINGBAS_LIBRARY_BASE`/`MATHIEEESINGTRANS_LIBRARY_BASE` (`0x27B0`/`0x29B0`, following the existing chunked-`0x200`-per-library convention) landed *past* `TRAP_TABLE_END` (`0x2600` at the time), which is where guest code/data starts loading -- an oversight caught by a full test-suite run, not by inspection: `dispatch::tests::small_stack_overflow_from_deep_pushes_is_detected` and `dosdatestr::tests::end_to_end_str_to_date_parses_date_and_time` both failed with unrelated-looking errors (`EmptyContinuationStack`, a date-parsing assertion) because `write_library_node` for the two new bases was overwriting bytes of those tests' own loaded guest programs. Fixed by growing `backend::TRAP_TABLE_SIZE` from `0x2600` to `0x2A00` (the same "grow it by one more chunk" pattern already used seven times for every previous real library base, one more doc-comment entry added to the existing list) -- exactly the intended fix mechanism, just not applied preemptively. Lesson: when adding a new real library base, always confirm the new address is still below the *current* `TRAP_TABLE_END`, not just "next after the previous highest constant" -- and always run the full workspace suite (not just the new code's own tests) before considering a new-library addition done, since a collision here manifests as an unrelated-looking failure in a totally different module's test.

Six new unit tests in `mathlibs.rs` (arithmetic natural order, fix/flt round trip, ceil sign, the corrected `Pow` base/exponent order, `Tieee`/`Fieee` identity, NaN canonicalization). Confirmed directly against the real `math_single_gcc`/`math_single_trans_gcc` binaries: only the already-understood `vamos`-side NaN-sign divergence (issue #52) and, for trans, the same 1-ULP transcendental rounding noise `math_fast_trans` already has (issue #53's residual) remain -- both added to `tools/compare_amitools_suite.py`'s `KNOWN_DIVERGENCES`/`CORPUS`, replacing its old "deferred, real gap" note. Full suite: 729 passing, clippy/fmt clean.
